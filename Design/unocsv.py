...
### Spreadsheet
fmts.add('spreadsheet', 'csvd', 'csv', 'Custom Text CSV', 'Text - txt - csv (StarCalc)') ### 16
        ...
        try:
            opts, args = getopt.getopt (args, 'c:Dd:e:f:hi:Llo:np:s:T:t:vV',
                ['connection=', 'debug', 'doctype=', 'export=', 'format=',
                 ...
                 'outputpath', 'page-size=', 'password=', 'pipe=', 'port=', 'server=',
...
unoconv options:
  ...
  -p, --port=port          specify the port (default: 2002)
      ...
      --page-size=string   specify a page size and orientation for the output file
                             eg. --page-size A3,LANDSCAPE (default: A4,PORTRAIT)
...
#            #print dir(filter), dir(filter.format)
    ...
    def convert(self, inputfn):
        ...
        try:
            ...
            ### Load inputfile
            if outputfmt.filter == 'Text - txt - csv (StarCalc)':
                # If we plan to export a CSV file don't set Hidden=True - Sheet activation does not work properly when Hidden is specified.
                # Although the sheet does become the active sheet, it's not the sheet that gets saved if the spreadsheet is loaded with Hidden=True
                inputprops = UnoProps(ReadOnly=True, UpdateDocMode=QUIET_UPDATE)
            else:
...
#                print "Import filter options: %s" % op.importfilteroptions
            ...
            if op.pagesize:
                try:
                    info(1, "Page Size: %s" % op.pagesize)
                    ...
                    paperSize,paperOrientation = op.pagesize.split(",")
...
#                print "Export filter options: %s" % op.exportfilteroptions
            ...
            if not op.stdout:
                ...
                if not op.output:
                    ...
                    if op.output.endswith('/'):
                        outputfn = realpath(op.output, os.path.basename(outputfn) + os.extsep + outputfmt.extension)
                    else:
                        outputfn = realpath(os.path.dirname(op.output), os.path.basename(op.output) + os.extsep + outputfmt.extension)
            ...
            try:
                if outputfmt.filter == 'Text - txt - csv (StarCalc)':
                    if outputfmt.name == 'csvd':
                        info(2, "custom CSV multi-sheet output")
                        ...
                        for i in range(oSheets.Count):
                                ...
                                if not op.output:
                                    sheetoutputfn = shname + os.extsep + outputfmt.extension
                                else:
                                    if op.output.endswith(os.pathsep):
                                        sheetoutputfn = realpath(op.output, shname + os.extsep + outputfmt.extension)
                                    else:
                                        sheetoutputfn = realpath(os.path.dirname(op.output), shname + os.extsep + outputfmt.extension)
                                outputurl = unohelper.absolutize( self.cwd, unohelper.systemPathToFileUrl(sheetoutputfn))
                                document.storeToURL(outputurl, tuple(outputprops))
                    else:
                        info(2, "CSV multi-sheet output")
                        (outputfn, ext) = os.path.splitext(inputfn)
                        oSheets = document.Sheets
                        for i in range(oSheets.Count):
                                sheet = oSheets.getByIndex(i)
                                document.CurrentController.setActiveSheet(sheet)
                                if not op.output:
                                    sheetoutputfn = outputfn + "-" + str(i) + os.extsep + outputfmt.extension
                                else:
                                    if op.output.endswith(os.pathsep):
                                        sheetoutputfn = realpath(op.output, os.path.basename(outputfn) + "-" + str(i) + os.extsep + outputfmt.extension)
                                    else:
                                        sheetoutputfn = realpath(os.path.dirname(op.output), os.path.basename(op.output) + "-" + str(i) + os.extsep + outputfmt.extension)
                                outputurl = unohelper.absolutize( self.cwd, unohelper.systemPathToFileUrl(sheetoutputfn))
                                document.storeToURL(outputurl, tuple(outputprops))
                else:
                        document.storeToURL(outputurl, tuple(outputprops) )
