...
class SpriteSheetMarginFix(object):
    def __init__(self, filename, tilewidth, tileheight, spacing, margin, outputname):
        self.margin = margin
...
if __name__ == "__main__":
    ...
    argv = sys.argv[1:]
    try:
        opts, args = getopt.getopt(argv, "f:x:y:m:s:o:", ["filename=","tilewidth=","tileheight=","margin=","spacing=","output="])
        for opt, arg in opts:
    ...
    if filename == None or tilewidth == 0 or tileheight == 0:
        help()
    if outputname == None:
    	outputname = "fixed-%s" % self.filename

    fix = SpriteSheetMarginFix( filename=filename, margin=margin, spacing=spacing, tilewidth=tilewidth, tileheight=tileheight, outputname=outputname)
    fix.fix_image()
